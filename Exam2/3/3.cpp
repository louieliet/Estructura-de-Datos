...
void DList::searchNShow(string pNombre){
    ...
    string* arryizq = new string[aVec];
    string* arryder = new string[aVec];
	if(aHead){
		...
		for(int i = 0; i<aVec;i++){
			if(aux && aux->sPrev){
				arryizq[i] = aux->sPrev->sNombre;
				aux = aux->sPrev;
			}else{
				
				arryizq[i] = "No hay vecino";
			}
			if(auxder && auxder->sNext ){
				arryder[i] = auxder->sNext->sNombre;
				auxder = auxder->sNext;
			}else{
				arryder[i] = "No hay vecino";
			}
		}
		
		for(int k = aVec-1; k >= 0; k--){
...
int main()
{
    ...
    lLista.searchNShow("CC");
    lLista.searchNWrite("CC");
