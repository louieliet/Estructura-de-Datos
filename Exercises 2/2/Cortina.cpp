...
void DLVer::push(string pMain, string pExtra)
{
    if (aHead == NULL) {
        ...
        if (pMain <= aHead->sPlatillo)
            push_front(pMain, pExtra);
        else if (pMain >= aTail->sPlatillo)
...
{
    if (aHead) {
        if (aHead->sPlatillo == pMain)
            pop_front();
        else if (aTail->sPlatillo == pMain)
            pop_back();
        else {
...
void DLVer::repr(bool pRev)
{
    if (aHead) {
        ...
        while (lTemp) {
            cout << " > ";
            cout << lTemp->sPlatillo << ", con votos: " << lTemp->sVotos;
...
void DLVer::max(){
    if(aHead){
        ...
        cout << pMax->sPlatillo << pMax->sVotos;
    }
}
            ...
            while (lTemp) {
                ...
                if (lTemp->sVer) {
                    ...
                    while (lItem = lTemp->sVer->get()) {
                        lFile << "\t" << lItem->sPlatillo << "|" << lItem->sPlatillo << endl;
                    }
                }
                lTemp = (pRev == false ? lTemp->sNext : lTemp->sPrev);
            }
