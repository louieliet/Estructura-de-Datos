...
public:
    DList(bool pFrec = false, int vecino = 0);
    ~DList(void);
    ...
    void searchNShow(string pNombre);
    void searchNWrite(string pNombre);
...
DList::DList(bool pFrec, int vecino)
{
    aHead = NULL;
    aTail = NULL;
    aCurr = NULL;
...
void DList::searchNShow(string pNombre){
	
	if(aHead){	
        vector<string> lLeft;
        vector<string> lRight;
		PDNODE lTemp = find(pNombre);
		PDNODE lTempRight = lTemp;
		PDNODE lTempLeft = lTemp;
		
		for(int i = 0; i<aNeigh;i++){
			if(lTempRight && lTempRight->sPrev){
				lLeft.push_back(lTempRight->sPrev->sNombre);
				lTempRight = lTempRight->sPrev;
			} else lLeft.push_back("N/A");
			
			if(lTempLeft && lTempLeft->sNext ){
				lRight.push_back(lTempLeft->sNext->sNombre);
				lTempLeft = lTempLeft->sNext;
			} else lRight.push_back("N/A");
		}
        if(lTemp){
            for(int i = aNeigh-1; i >= 0; i--){
                cout<< lLeft[i] <<", ";   
            }
            cout<<"*"<<lTemp->sNombre<<"*, ";
            for(int i = 0; i< aNeigh; i++){
...
void DList::searchNWrite(string pNombre){
	...
	if(aHead){	
		...
		PDNODE lTempRight = lTemp;
		PDNODE lTempLeft = lTemp;
        ofstream fileNghbr("Neighbour.txt");
...
int main()
{
    DList lLista = DList(false, 35);
    
    lLista.push("Juan");
    lLista.push("Ariana");
    lLista.push("Diego");
    lLista.push("Francisco");
    lLista.push("Pepe");
    lLista.push("Emiliano");
    lLista.push("Alejandro");
    lLista.push("Marcia");
    lLista.push("Cantu");
    lLista.push("Carlos");
    lLista.push("Macario");
    ...
    lLista.repr();    
    lLista.searchNShow("Emiliano");
    lLista.searchNWrite("Emiliano");
